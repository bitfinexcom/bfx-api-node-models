...
class Order extends Model {
  /**
   * @param {Order~Data} [data={}] - order data
   * @param {object} [apiInterface] - saved for a later call to
   *   `registerListeners()``
   */
  ...
  /**
   * @param {Order~Data|Order~Data[]} data - data to convert to POJO
   * @returns {Order~ObjectData} pojo
   */
   ...
   * @param {(
   *   bitfinex-api-node.WSv2|
   *   bfx-api-node-rest.RESTv2
   * )} [apiInterface] - optional ws or rest, defaults to internal instance
   *
   ...
   *  bitfinex-api-node.WSv2|
   *  bfx-api-node-rest.RESTv2
   * )} [apiInterface] - optional ws defaults to internal ws
   */
   ...
   *  bitfinex-api-node.WSv2|
   *  bfx-api-node-rest.RESTv2
   * )} [apiInterface] - defaults to internal ws
   *
   * @returns {Promise} p
   */
   ...
   *
   * @throws {Error} fails if the order ID/CID/GID do not match
   *
   ...
   *
   * @see {@link Order~getLastFillAmount}
   * @see {@link Order~isPartiallyFilled}
   */
  resetFilledAmount () {
    this._lastAmount = this.amount
  }
   ...
   * @private
   *
   * @param {Order~ArrayData} order - incoming order
   */
   ...
   *
   * @param {Order~ArrayData} arr - order in ws2 array format
   ...
   *
   * @param {Order~Data|Order~Data[]} data - instance(s) to validate
...
Order.flags = {
  /**
   * Order cancels order (16384). Requires `priceAuxLimit` and `cidOCO` to be
   * set, resolved to `price_oco_stop` and `cid_oco` on the generated `on`
   * packet.
   */
  OCO: 2 ** 14,

  /**
   * Post-only (4096) orders are cancelled if they would execute immediately
   */
  POSTONLY: 2 ** 12,

  /**
   * Hidden (64) orders are inserted into the book, not visible to other
   * traders, and always pay the **taker** fee.
   */
  HIDDEN: 2 ** 6,

  /**
   * Excludes variable rate (524288) funding
   */
  NO_VR: 2 ** 19,

  /**
   * Position-close (512) orders are cancelled if they would not close an
   * open position.
   */
  POS_CLOSE: 2 ** 9,

  /**
   * Reduce-only (1024) orders are cancelled if they would open or increase
   * the size of an existing position.
   */
  REDUCE_ONLY: 2 ** 10
}
