...
class Order extends Model {
  ...
  getLastFillAmount () {
    const { status } = this
    const events = status.split(';').map(_trim)
    let [ lastEvent ] = events

    if (lastEvent.match(/:/)) { // handle ..was: ...
      lastEvent = lastEvent.split(':')[1]
    }

    if (lastEvent.indexOf('CANCELED') !== -1) {
      return 0
    } else if (
      (lastEvent.indexOf('PARTIALLY FILLED') !== -1) ||
      (lastEvent.indexOf('EXECUTED') !== -1)
    ) {
      const amountStr = lastEvent.split('(')[1]
      const fillAmount = Number(amountStr.slice(0, amountStr.length - 1))
