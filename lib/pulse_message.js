...
class PulseMessage extends Model {
  /**
   * @param {PulseMessage~Data|PulseMessage~Data[]} data - data
   */
  constructor (data = {}) {
    super({ data, fields })
  }
  ...
  /**
   * @param {PulseMessage~Data|PulseMessage~Data[]} data - data to convert
   * @returns {PulseMessage~ObjectData} pojo
   */
  ...
  toString () {
    const {
      id, mts, userID, title, content, isPin, isPublic, tags, attachments,
      likes, userLiked
    } = this
   ...
   *
   * @param {PulseMessage~Data|PulseMessage~Data[]} data - instance(s) to
   *   validate
